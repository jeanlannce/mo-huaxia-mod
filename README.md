...
> ⚠️ 本仓库**仅包含**文本规则文件与说明文档，**不含**《红色警戒 2：尤里的复仇》或 Mental Omega 的游戏本体、美术资源。使用前请自备正版游戏与 Mental Omega 3.3.6。

## 内容

| 文件 | 说明 |
|---|---|
| `rulesmo.ini` | 主规则文件（华夏专属修改在此基础上的子文件覆盖） |
...
- 华夏 AI 专属编队系统（v1.4）：15 个编队按科技阶段（TL1-9）出兵；原版 AI 编队完整保留（高位键追加）
- 大量华夏专属克隆单位/建筑（v1.6）：海豹 AWP 狙、斗牛犬双形态、猎狼犬直升机、维修悍马、迷彩碉堡、千里马运输直升机等

## ⚠️ 已知问题（重要）

- **遭遇战初始部队请保持 0**：华夏（Huaxia）为自定义国家，不在 CnCNet 客户端初始部队的国家映射内——若在遭遇战设置里选择初始部队数量（如 50），可能导致 AI 开局即被判定击败。请将"初始部队"设为 0（UnitCount=0）。
